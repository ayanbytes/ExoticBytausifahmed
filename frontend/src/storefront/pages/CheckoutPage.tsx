        ...
        <div className="section-container py-10 max-w-5xl mx-auto">
          ...
          {step === 'details' && (
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
              <form onSubmit={handleDetailsSubmit} className="lg:col-span-3 space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="text-label text-xs text-mid block mb-2">Full Name *</label>
                    <input
                      type="text"
                      value={details.name}
                  ...
                  <div>
                    <label className="text-label text-xs text-mid block mb-2">Phone *</label>
                    <input
                      type="tel"
                      value={details.phone}
                ...
                <div>
                  <label className="text-label text-xs text-mid block mb-2">Email (optional)</label>
                  <input
                    type="email"
                    value={details.email}
                ...
                <div>
                  <label className="text-label text-xs text-mid block mb-2">Delivery Address *</label>
                  <textarea
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  <div>
                    <label className="text-label text-xs text-mid block mb-2">City *</label>
                    <input
                      type="text"
                      value={details.city}
                  ...
                  <div>
                    <label className="text-label text-xs text-mid block mb-2">State *</label>
                    <input
                      type="text"
                      value={details.state}
                  ...
                  <div>
                    <label className="text-label text-xs text-mid block mb-2">Pincode *</label>
                    <input
                      type="text"
                ...
                <div>
                  <label className="text-label text-xs text-mid block mb-2">Order Notes (optional)</label>
                  <textarea
                    value={details.notes}
          ...
          {step === 'review' && (
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
              <div className="lg:col-span-3 space-y-6">
                ...
                <div className="border border-graphite p-6 space-y-3">
                  <h3 className="text-label text-xs text-gold mb-4">Delivery Details</h3>
                  ...
                  <button onClick={() => setStep('details')} className="text-xs text-gold hover:underline mt-2">
                    Edit Details
                  </button>
                </div>

                {/* WhatsApp CTA */}
                <div className="border border-graphite p-6 bg-charcoal">
                  ...
                  <motion.button
                    ...
                    className="w-full flex items-center justify-center gap-3 py-4 bg-[#25D366] hover:bg-[#20bf5a] text-white text-label text-sm font-semibold tracking-wider transition-colors disabled:opacity-50"
                    whileTap={{ scale: 0.98 }}
                  >
                    {isSubmitting ? (
...
function OrderSummary() {
  ...
  return (
    <div className="border border-graphite p-6 space-y-4 sticky top-24">
      <h3 className="text-label text-xs text-gold mb-4">Order Summary</h3>
        ...
        {items.map((item) => (
          <div key={item.id} className="flex gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm text-cream font-light leading-tight">{item.product_name}</p>
              {item.variant_size && <p className="text-xs text-mid">{item.variant_size}</p>}
              <p className="text-xs text-silver mt-0.5">×{item.quantity}</p>
            </div>
            <p className="text-sm text-cream">₹{(item.unit_price * item.quantity).toLocaleString('en-IN')}</p>
          </div>
        ))}
      </div>
